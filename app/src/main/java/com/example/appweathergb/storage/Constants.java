package com.example.appweathergb.storage;

public interface Constants {
    int REQUEST_CODE_MAIN_AND_SEARCH_ACTIVITY = 777;
    String KEY_SEARCH_ACTIVITY = "keySearch";
    String KEY_MAIN_ACTIVITY = "keyMain";
    int RESULT_COD_SEARCH_VIEW = 999;
    int transparent = -15108398;
    int alpha0 = 0;
    char celsius = 0x00B0;
    String failConnection = "Fail connection";
    String requestMethodGet = "GET";
    String failURI = "Fail URI";
    int SETTING_CODE = 88;
    int subtract = 273;
    String KEY_LOCATION_SERVICE = "keyLocationService";
    String CHANNEL_ID_NOTIF = "id2";

}
